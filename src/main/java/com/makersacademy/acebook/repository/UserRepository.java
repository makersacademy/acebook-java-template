...
public interface UserRepository extends CrudRepository<User, Long> {
  ...
  @Query(
    value = "SELECT friendship_id FROM friends WHERE (requester_id = ?1 AND requestee_id = ?2) OR (requestee_id = ?1 AND requester_id = ?2)",
    nativeQuery = true)
  Long getFriendshipId(Long userId1, Long userId2);
  
  @Query(
    value = "SELECT * FROM users WHERE NOT users.id IN (SELECT DISTINCT users.id FROM users JOIN friends ON friends.requester_id = users.id OR friends.requestee_id = users.id WHERE friends.requestee_id = ?1 OR friends.requester_id = ?1) AND users.id != ?1",
    nativeQuery = true)
  ...
  @Modifying
  @Query(
    value = "UPDATE friends SET request_status = 'accepted' WHERE (requester_id = ?1 AND requestee_id = ?2) OR (requestee_id = ?1 AND requester_id = ?2) AND request_status = 'pending'",
    nativeQuery = true)
