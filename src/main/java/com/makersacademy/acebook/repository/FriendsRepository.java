...
public interface FriendsRepository extends CrudRepository<Friend, Long> {
 ...
 // @Query(value = "SELECT users.id, users.username FROM users INNER JOIN friends
 // ON users.id = friends.request_from WHERE friends.status_code = TRUE AND
 // friends.request_to = ?1 UNION SELECT users.id, users.username FROM users
 // INNER JOIN friends ON users.id = friends.request_to WHERE friends.status_code
 // = TRUE AND friends.request_from = ?1", nativeQuery = true)
